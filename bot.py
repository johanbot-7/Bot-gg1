...
modo_mantenimiento = cargar_json(MANTENIMIENTO_FILE, False)
    ...
    if not esta_autorizado(uid):
        ...
        if intentos[uid_str] >= 3:
            ...
            await context.bot.send_message(
                chat_id=ADMIN_ID,
                text=f"""🚨 USUARIO BLOQUEADO

👤 Nombre: {user.first_name}
🔗 Usuario: {username}
🆔 ID: {uid}

Intentó acceder sin autorización 3 veces."""
            )

        return

    # 🔧 MANTENIMIENTO
    if modo_mantenimiento and not es_admin(uid):
        await update.message.reply_text(
            "🔧 El bot está en mantenimiento temporal. Por favor intenta más tarde."
        )
        return
...
async def manejar(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # 🔥 PRIORIDAD: SI ESTÁ EN FORMULARIO → IGNORAR COMANDOS
    ...
    # 🔹 SI NO ESTÁ EN FORMULARIO → VALIDAR COMANDO
    ...
    if texto == "18":
        ...
        if not tiempo or tiempo < time.time():
            await update.message.reply_text("❌ Solo admin esta permitido.")
            return
    ...
    if texto == "19":
        ...
        if not es_admin(uid):
            await update.message.reply_text("❌ Solo admin esta permitidoo")
            return
...
async def color(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("aleron_"):
        ...
        await query.edit_message_text(f"Alerón seleccionado: {color}")
...
async def activar17(update:Update,context:ContextTypes.DEFAULT_TYPE):
    ...
    await query.edit_message_text("Función 18 activada")
...
async def quitar_funcion17(update:Update,context:ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text("Función 18 eliminada")
...
async def eliminar(update:Update,context:ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text("Usuario eliminado correctamente.")
...
"""
            ...
            fotos = await context.bot.get_user_profile_photos(user_id)

            if fotos.total_count > 0:
                foto = fotos.photos[0][-1].file_id  # mejor calidad
            ...
            await update.message.reply_text(f"⚠ Error con usuario {uid}")
...
"""
    ...
    if msg.sticker:
        ...
        await context.bot.send_message(ADMIN_ID, texto, reply_markup=keyboard)

    elif msg.photo:
        file = msg.photo[-1].file_id
        await context.bot.send_photo(ADMIN_ID, file, caption=texto, reply_markup=keyboard)

    elif msg.voice:
        file = msg.voice.file_id
        await context.bot.send_voice(ADMIN_ID, file, caption=texto, reply_markup=keyboard)

    elif msg.audio:
        file = msg.audio.file_id
        await context.bot.send_audio(ADMIN_ID, file, caption=texto, reply_markup=keyboard)

    elif msg.video:
        file = msg.video.file_id
        await context.bot.send_video(ADMIN_ID, file, caption=texto, reply_markup=keyboard)

    elif msg.document:
        file = msg.document.file_id
        await context.bot.send_document(ADMIN_ID, file, caption=texto, reply_markup=keyboard)

    await update.message.reply_text(
        "⚠ Acción identificada como extraña (Notificada al administrador), usa el bot correctamente."
    )
async def bloquear_usuario(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        await context.bot.send_message(
            chat_id=int(uid),
            text="🚫 Has sido bloqueado por actividad sospechosa.\n\nSi crees que es un error, contacta al administrador."
        )
    except:
        pass

    # 🔔 CONFIRMACIÓN AL ADMIN
    await query.edit_message_text("🚫 Usuario bloqueado correctamente")
# ------------------------
# BOTON MANTENIMIENTO
# ------------------------
...
print("juhr3koiudyhh9764yiwwuhygdbhsju87364ty83746fdhjuidfhgbi3u7ygfhkmdjnhfbdjskisjdcfhydusiosi9d8u7y6d3890swidchuygt6y7w8uiaeshdy6e78sijwhegfvbhnjdkijufhyueiw8734t5yhjkoe9i8ury75ghrjki398ut4yghfnjmshdgfbnmlpñaos09i8duy7ctgfbnjko987ey6rtfgbhnjksoiu8y7dtcgbhndksi83u76t45fgchbnjxkoi9283u74y6tycghxnjki8736t4ghnjki873y6g")
